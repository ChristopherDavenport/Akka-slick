...
trait BaseDal[T,A] {
  ...
  def findByFilterToOne[C : CanBeQueryCondition](f: (T) => C) : Future[Option[A]]
...
class BaseDalImpl[T <: BaseTable[A], A <: BaseEntity](tableQ: TableQuery[T])(implicit val db: JdbcProfile#Backend#Database,implicit val profile: JdbcProfile) extends BaseDal[T,A] with Profile with DbModule {
  ...
  override def findByFilterToOne[C: CanBeQueryCondition](f: (T) => C): Future[Option[A]] = {
    db.run(tableQ.withFilter(f).result.headOption)
  }
